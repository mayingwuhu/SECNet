...
def mk_video_ret_dataloader(anno_path, lmdb_dir, cfg, tokenizer, is_train=True):
    ...
    if 'lsmdc' in cfg.train_datasets[0]['name']:
        video_fmt = '.avi'
    else:
        video_fmt = '.mp4'
    ...
    for batch in val_loader:
        ...
        for idx in range(n_mini_batches):
            ...
            for clip_idx in range(num_clips):
                ...
                # print("mini_batch['visual_inputs'].shape",mini_batch['visual_inputs'].shape)
                # #mini_batch的key['text_input_ids', 'text_input_mask', 'labels', 'n_examples_list', 'visual_inputs']
                # print("mini_batch[text_input_ids]:", mini_batch['text_input_ids'].shape)#shape:[64, 40]
                # print("mini_batch['text_input_mask']:",mini_batch['text_input_mask'].shape)
                # print("mini_batch['labels']:",mini_batch['labels'])#None
                # print("mini_batch['n_examples_list']:", mini_batch['n_examples_list'])#[64]
                if cfg.fp16:
