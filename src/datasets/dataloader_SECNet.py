...
class PrefetchLoader(object):
    ...
    def __iter__(self):
        ...
        while batch is not None:
            ...
            if is_tuple:
                task, batch = batch
            # LOGGER.info("输出tensor维度:", batch["visual_inputs_WH1"].size())
            batch["visual_inputs_WH1"] = batch["visual_inputs_WH1"].float()
            batch["visual_inputs_WH2"] = batch["visual_inputs_WH2"].float()
            batch["visual_inputs_Where"] = batch["visual_inputs_Where"].float()

            if self.img_normalize is not None:
                batch["visual_inputs_WH1"] = self.img_normalize(
                    batch["visual_inputs_WH1"])
                batch["visual_inputs_WH2"] = self.img_normalize(
                    batch["visual_inputs_WH2"])
                batch["visual_inputs_Where"] = self.img_normalize(
                    batch["visual_inputs_Where"])
