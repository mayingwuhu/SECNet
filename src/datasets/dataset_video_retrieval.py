...
class VideoRetrievalCollator(object):
    ...
    def collate_batch(self, batch):
        ...
        # group data
        text_examples = flat_list_of_lists([d["examples"] for d in batch])#[{'txt_str':'文字描述','id':0},{'txt_str':'文字描述','id':1},...]
        ...
        if "id" in text_examples[0]:
            caption_ids = [d["id"] for d in text_examples]  # (B, ) caption_ids:[0,1,....,1000]
        else:
            caption_ids = None
        collated_batch = dict(
...
class AlproVideoRetrievalEvalDataset(AlproBaseDataset):
    ...
    def __init__(self, datalist, tokenizer, img_lmdb_dir,
        ...
        for i, d in enumerate(self.datalist):
            assert i == d["id"]
        self.gt_cap_id2vid_id = {d["id"]: d["vid_id"] for d in datalist}#{0:'video9770',1:'video9771'}
        self.cap_id2data = {d["id"]: d for d in datalist}#{0: {'id': 0, 'txt': 'a person is connecting something to system', 'vid_id': 'video9770'}, 1: {'id': 1, 'txt': 'a little girl does gymnastics', 'vid_id': 'video9771'}}
    ...
    def _prepare_batches_by_video(self):
        ...
        '''demo_test'''
        #text_batch = {'vid_id':None,'examples':[{'text_str': 'a person is connecting something to system', 'id': 0}],'n_examples':1,'ids':[0]}
        print("text_batch:", text_batch)
        print("text_batch.keys", text_batch.keys())
